    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #2c2416 0%, #4a3728 60%, #2c2416 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "Georgia, serif" }}>
      <div style={{ background: "#faf8f5", borderRadius: 20, padding: 40, maxWidth: 420, width: "100%", boxShadow: "0 32px 80px rgba(0,0,0,0.4)", textAlign: "center" }}>
        ...
        <div style={{ textAlign: "left", marginBottom: 18 }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} placeholder="e.g. Sarah" autoFocus
...
// ── Note preview helper ────────────────────────────────────────────────────────
function NotePreview({ item, note, color }) {
  if (!note) return null;
  const entries = [];

  if (item.modal === "vendor") {
    if (note.name)  entries.push(`👤 ${note.name}`);
    if (note.email) entries.push(`✉ ${note.email}`);
    if (note.phone) entries.push(`📞 ${note.phone}`);
    if (note.cost)  entries.push(`💰 ${note.cost}`);
    if (note.notes) entries.push(`"${note.notes.slice(0, 50)}${note.notes.length > 50 ? "…" : ""}"`);
  } else if (item.modal === "budget") {
    if (note.min) entries.push(`Min: ${note.min}`);
    if (note.max) entries.push(`Max: ${note.max}`);
  } else if (item.modal === "custom" && item.fields) {
    item.fields.forEach(f => {
      if (note[f.key]) entries.push(f.type === "url" ? `🔗 Link saved` : `${note[f.key]}`);
    });
  } else if (item.modal === "delegates" && note.rows) {
    entries.push(`${note.rows.filter(r => r.name).length} delegate(s)`);
  } else if (item.modal === "vendors_table" && note.rows) {
    entries.push(`${note.rows.filter(r => r.name).length} contact(s)`);
  }
  ...
  return (
    <div style={{ padding: "0 14px 11px 50px", display: "flex", flexWrap: "wrap", gap: "4px 14px", alignItems: "center" }}>
      {entries.map((e, i) => <span key={i} style={{ fontSize: 12, color: "#6a5a4a", fontStyle: e.startsWith('"') ? "italic" : "normal" }}>{e}</span>)}
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onValue(ref(db, "checkedItems"), snap => { setCheckedItems(snap.val() || {}); setLoading(false); });
    return () => unsub();
  }, []);

  useEffect(() => {
    const unsub = onValue(ref(db, "notes"), snap => { setSavedNotes(snap.val() || {}); });
    return () => unsub();
  }, []);

  useEffect(() => {
    if (!currentUser) return;
    const pk = currentUser.name.replace(/[.#$[\]]/g, "_");
    const presRef = ref(db, `presence/${pk}`);
    set(presRef, { name: currentUser.name, role: currentUser.role, lastSeen: Date.now() });
    const unsub = onValue(ref(db, "presence"), snap => setActiveUsers(snap.val() || {}));
    return () => { unsub(); set(presRef, null); };
  }, [currentUser]);
  ...
  const sectionProgress = (section) => {
    const done = section.items.filter(item => checkedItems[`${section.id}-${item.id}`]).length;
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {onlineUsers.length > 0 && <span style={{ color: "#7a6a5a", fontSize: 11, marginLeft: 8 }}>{onlineUsers.map(u => u.name).join(", ")} online</span>}
            </div>
            <button onClick={() => { localStorage.removeItem(USER_KEY); setCurrentUser(null); }} title="Switch user"
          ...
          <div key={section.id} style={{ maxWidth: 720, margin: "0 auto", padding: "26px 20px 60px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {section.items.map((item) => {
                ...
                return (
                  <div key={item.id} style={{ background: isDone ? `${section.color}10` : "#fff", border: `1px solid ${isDone ? section.color + "55" : "#ede9e1"}`, borderRadius: 10, overflow: "hidden", transition: "all 0.2s" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "13px 14px" }}>
                      {/* Checkbox */}
                      <div onClick={() => handleCheck(section, item)}
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {isDone && checkData && (
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                            ...
                            <span style={{ fontSize: 11, color: "#9a8a7a" }}>{checkData.completedBy} · {checkData.completedAt}</span>
                          </div>
                        )}
                      </div>

                      {/* Note button — only show if item has a modal */}
                      {hasModal && (
                        <button onClick={() => setModal({ key, item, phase: section.label, color: section.color })}
                    ...
                    {/* Note preview */}
                    {note && <NotePreview item={item} note={note} color={section.color} />}
                  </div>
                );
              })}
            </div>
